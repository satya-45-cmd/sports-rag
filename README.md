...
├── chroma_db/              # generated automatically, not committed to git
...
- Deploy. The first load will take a little longer than usual since it
  builds the vector index automatically (see `app.py`) and downloads the
  embedding model — after that it's fast.
